...
export const PaginaCard: React.FC<PaginaCardProps> = ({ pagina, jornalId }) => {
  ...
  // Mock top 10 operadores para demonstração
  const topOperadores = [
        ...
        <div className="space-y-4">
          ...
          >
            <div className="space-y-3 pt-2 border-t border-gray-100">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700 mb-2">Top Operadores:</p>
                <div className="space-y-1">
                  {topOperadores.slice(0, 5).map((operador, index) => (
                    <div key={index} className="text-xs text-gray-600 flex justify-between">
                      <span>{operador}</span>
                      <span className="text-green-600">R$ {(Math.random() * 5000 + 1000).toFixed(0)}</span>
                    </div>
                  ))}
                </div>
              </div>
